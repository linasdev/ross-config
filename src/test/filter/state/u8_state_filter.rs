...
fn u8_increment_state_filter_initial_zero_test() {
    ...
    state_manager.set_value(0, Value::U8(0));

    let mut filter = U8IncrementStateFilter::new(0);

    assert_eq!(filter.filter(&Value::None, &mut state_manager), true);
    assert_eq!(*state_manager.get_value(0).unwrap(), Value::U8(1));
    assert_eq!(filter.filter(&Value::None, &mut state_manager), true);
    assert_eq!(*state_manager.get_value(0).unwrap(), Value::U8(2));
}

#[test]
fn u8_increment_state_filter_initial_seven_test() {
    let mut state_manager = StateManager::new();
    state_manager.set_value(0, Value::U8(7));

    let mut filter = U8IncrementStateFilter::new(0);

    assert_eq!(filter.filter(&Value::None, &mut state_manager), true);
    assert_eq!(*state_manager.get_value(0).unwrap(), Value::U8(8));
    assert_eq!(filter.filter(&Value::None, &mut state_manager), true);
    assert_eq!(*state_manager.get_value(0).unwrap(), Value::U8(9));
}
...
fn u8_increment_state_filter_value_has_bad_type_test() {
    let mut state_manager = StateManager::new();
    state_manager.set_value(0, Value::U8(VALUE_1));
...
fn u8_increment_state_filter_state_value_has_bad_type_test() {
    let mut state_manager = StateManager::new();
    state_manager.set_value(0, Value::U32(VALUE_2));
